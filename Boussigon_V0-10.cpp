                   ...
                   const sf::Vector2f& dist, const float mass_mult)
{
    ...
    const float abs_accel{grav_mult*obje_circ.m_mass*(1 - abs_dist/(mass_mult*obje_circ.m_mass))};
...
int main()
{
    ...
    const std::string program_name{"Boussigon V0.10"};
    ...
    const sf::Vector2f rect_position_1{0.25f*window_x, 0.25f*window_y};    
    object_circle object_1{rect_radius_1, rect_color_1, rect_position_1};
    
    const float rect_radius_2{25.0f};    
    const sf::Color rect_color_2{blue};    
    const sf::Vector2f rect_position_2{0.75f*window_x, 0.75f*window_y};     
    object_circle object_2{rect_radius_2, rect_color_2, rect_position_2};
    ...
    while (window.isOpen())
    {
        ...
        show_object_circle(window, object_1);
        
        show_object_circle(window, object_2);
        ...
        for (int count{0}; count < count_max; ++count)
        {
            ...
            window_wrap(window_sizes, player);
                    
        }
        
        collision_color(player, object_1, colli_color, circ_color, delta_fuel);
        
        collision_color(player, object_2, colli_color, circ_color, delta_fuel);
